    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
          <h2 className="mb-3 text-sm font-semibold text-white">Available GLPI tables</h2>
          {loading ? <p className="text-xs text-slate-400">Loading table schema...</p> : tables.length === 0 ? <p className="text-xs text-slate-400">No GLPI tables found.</p> : (
            <div className="max-h-[620px] space-y-1 overflow-auto">
              {tables.map((table) => <button key={table} onClick={() => { setSelected(table); setError(null); setData(null); }} className={`block w-full rounded px-3 py-2 text-left text-xs ${selected === table ? 'bg-cyan-500/15 text-cyan-300' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>{table}</button>)}
            </div>
          )}
        </section>
