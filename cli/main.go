...
func main() {
	...
	if longRunning || commandName == "configure" {
		ui.Welcome()
	}
	if debug {
...
func configure() error {
	...
	for {
		...
		if err != nil {
			...
			if !askYesNo("Retry configuration? [Y/n]: ", true) {
				return errors.New("MySQL configuration was not saved")
			}
			existing = &value
			continue
		}
		if !result.DatabaseExists {
			...
			if choice != "1" {
				return errors.New("database does not exist; configuration was not saved")
			}
			if err := mysqlcheck.CreateDatabase(context.Background(), value); err != nil {
				return err
			}
			result.DatabaseExists = true
		}
		if !result.LooksLikeGLPI {
			ui.Warning(fmt.Sprintf("Database %q does not currently contain expected GLPI tables; continuing is allowed for a new GLPI installation.", value.Database))
		}
		...
		for {
			...
			if strings.TrimSpace(key) == "" {
				ui.Error("Gemini API key is required.")
				continue
			}
...
func start() error {
	...
	for _, dependency := range dependencies {
		if containsDependency(missing, dependency.Name) {
			ui.Warning(fmt.Sprintf("%s missing (required: %s)", dependency.Name, dependency.Required))
		} else {
			ui.Success(fmt.Sprintf("%s detected", dependency.Name))
		}
	}
	if len(missing) > 0 {
		...
		if !askYesNo("Install these components now? [y/N]: ", false) {
			return errors.New("installation cancelled by user; no dependency changes were made")
		}
		if err := installDependencies(missing); err != nil {
			return stageError("dependency installation", err)
		}
	}
	...
	if err != nil {
		return stageError("MySQL configuration", err)
	}
	...
	if err := ensureGeminiAPIKey(&value); err != nil {
		return stageError("Gemini API configuration", err)
	}
	value.CyberNexusHost = value.RiskNexusHost
	browserURLs, err := config.GenerateBrowserURLs(value.RiskNexusHost)
	if err != nil {
	...
	if err := config.Save(path, value); err != nil {
		return stageError("environment configuration", err)
	}
	ui.Success(fmt.Sprintf("Secure configuration saved at %s", path))

	ui.Step("[5/9] Configuring GLPI")
	databaseState, err := mysqlcheck.Check(context.Background(), value)
	if err != nil {
		return stageError("GLPI database inspection", err)
	}
	if !glpi.Detect(settings) {
		ui.Warning(fmt.Sprintf("GLPI was not detected at %s.", settings.InstallPath))
		if !askYesNo("Download and install the configured GLPI release now? [Y/n]: ", true) {
			return errors.New("GLPI installation cancelled by user")
		}
		if err := installGLPI(settings, value, !databaseState.LooksLikeGLPI); err != nil {
			return stageError("GLPI installation", err)
		}
	} else {
		ui.Success(fmt.Sprintf("Existing GLPI installation found at %s", settings.InstallPath))
		if !databaseState.LooksLikeGLPI {
			if err := installGLPI(settings, value, true); err != nil {
				return stageError("GLPI database initialization", err)
			}
		}
	}
	if err := configureApache(settings); err != nil {
		return stageError("Apache configuration", err)
	}
	if err := glpi.VerifyHTTP(context.Background(), settings); err != nil {
		return stageError("GLPI health check", err)
	}

	ui.Step("[6/9] Configuring GLPI Inventory")
	...
	ui.Step("[7/9] Configuring GLPI Agent")
	if _, err := exec.LookPath("glpi-agent"); err != nil {
		if err := runCommand("/", "apt-get", "install", "-y", "glpi-agent"); err != nil {
			return stageError("GLPI Agent installation", err)
		}
	}
	if err := glpi.WriteAgentConfig(settings); err != nil {
		return stageError("GLPI Agent configuration", err)
	}
	if err := runCommand("/", "systemctl", "enable", "--now", "glpi-agent"); err != nil {
		return stageError("GLPI Agent startup", err)
	}

	ui.Step("[8/9] Collecting and verifying inventory")
	if err := runCommand("/", "glpi-agent", "--debug", "--force"); err != nil {
		return stageError("inventory collection", err)
	}
	counts, err := mysqlcheck.Inventory(context.Background(), value)
	if err != nil {
		return stageError("inventory verification", err)
	}
	if counts.Computers == 0 {
		return stageError("inventory verification", errors.New("no computer inventory was received; inspect glpi-agent and Apache logs"))
	}
	...
	ui.Step("[9/9] Starting FastAPI and Next.js")
	if err := compose("up", "-d", "--build"); err != nil {
		return stageError("application services", err)
	}
	root, err := repositoryRoot()
	if err != nil {
		return stageError("Docker network discovery", err)
	}
	...
	if err := mysqlcheck.EnsureDockerNetworkAccess(context.Background(), value, subnet); err != nil {
		...
		if promptErr != nil {
			return stageError("MySQL Docker network authorization", promptErr)
		}
	...
	if err := waitHTTP("http://localhost:8000/", 30); err != nil {
		return stageError("FastAPI health check", err)
	}
	if err := waitHTTP("http://localhost:3000/", 30); err != nil {
		return stageError("Next.js health check", err)
	}
	if err := checkDockerServices(); err != nil {
		return stageError("Docker service health check", err)
	}
...
func ensureGeminiAPIKey(value *config.MySQLConfig) error {
	if strings.TrimSpace(value.GeminiAPIKey) != "" {
		ui.Success("Gemini API key already configured")
		return nil
	}

	ui.Step("[4/9] Configuring Gemini AI")
	if !term.IsTerminal(int(os.Stdin.Fd())) {
		return errors.New("Gemini API key is required for non-interactive execution. Configure GEMINI_API_KEY before running in non-interactive mode.")
	}
...
func configuredMySQL(path string) (config.MySQLConfig, error) {
	if current, err := config.Load(path); err == nil {
		if result, checkErr := mysqlcheck.Check(context.Background(), current); checkErr == nil {
			if result.DatabaseExists {
				ui.Success("Existing MySQL configuration is valid")
				return current, nil
			}
		}
	}
	for {
		...
		result, err := mysqlcheck.Check(context.Background(), value)
		if err != nil {
			ui.Error(fmt.Sprintf("MySQL connection failed: %v", err))
			if askYesNo("Retry configuration? [Y/n]: ", true) {
				continue
			}
			return config.MySQLConfig{}, errors.New("MySQL configuration cancelled")
		}
		if !result.DatabaseExists {
			ui.Warning(fmt.Sprintf("Database %q does not exist.", value.Database))
			if !askYesNo("Create it? [y/N]: ", false) {
				return config.MySQLConfig{}, errors.New("database creation declined")
			}
			if err := mysqlcheck.CreateDatabase(context.Background(), value); err != nil {
				return config.MySQLConfig{}, err
			}
...
func promptMySQLAdminCredentials() (mysqlcheck.AdminCredentials, error) {
	...
	if username == "" {
		return mysqlcheck.AdminCredentials{}, errors.New("MySQL administrative username is required")
	}
	fmt.Print("MySQL administrative password: ")
	password, err := readPassword()
	fmt.Println()
	if err != nil {
		return mysqlcheck.AdminCredentials{}, fmt.Errorf("read MySQL administrative password: %w", err)
	}
	if len(password) == 0 {
		return mysqlcheck.AdminCredentials{}, errors.New("MySQL administrative password is required")
	}
...
func installGLPI(settings glpi.Settings, value config.MySQLConfig, initializeDatabase bool) error {
	...
	if _, err := os.Stat(settings.InstallPath); os.IsNotExist(err) {
		if err := runCommand("/", "curl", "--fail", "--location", "--output", archive, archiveURL); err != nil {
			return err
		}
		if err := runCommand("/var/www", "tar", "-xzf", archive); err != nil {
			return err
		}
		if err := runCommand("/", "chown", "-R", "www-data:www-data", settings.InstallPath); err != nil {
			return err
		}
	}
...
func configureApache(settings glpi.Settings) error {
	...
	if err := os.WriteFile("/etc/apache2/sites-available/risknexus-glpi.conf", []byte(content), 0644); err != nil {
		return fmt.Errorf("write Apache site: %w", err)
	}
	if err := runCommand("/", "a2enmod", "rewrite"); err != nil {
		return err
	}
	if err := runCommand("/", "a2ensite", "risknexus-glpi.conf"); err != nil {
		return err
	}
...
func checkDockerServices() error {
	...
	if !strings.Contains(strings.ToLower(output), "running") && !strings.Contains(strings.ToLower(output), "up") {
		return fmt.Errorf("Docker Compose reports no running application services: %s", strings.TrimSpace(output))
	}
	return nil
}
...
func dockerNetworkSubnet(root, composeFile, configPath string) (string, error) {
	...
	if containerID == "" {
		return "", errors.New("Docker Compose did not return a backend container ID")
	}
	...
	if err := json.Unmarshal([]byte(strings.TrimSpace(networksJSON)), &networks); err != nil {
		return "", fmt.Errorf("decode backend Docker networks: %w", err)
	}
	for _, network := range networks {
		...
		if err := json.Unmarshal([]byte(strings.TrimSpace(ipamJSON)), &configs); err != nil {
			return "", fmt.Errorf("decode Docker network IPAM: %w", err)
		}
...
func repositoryRoot() (string, error) {
	...
	if _, statErr := os.Stat(filepath.Join(root, "deployment", "docker-compose.yml")); statErr == nil {
		return root, nil
	}
	parent := filepath.Dir(root)
	if _, statErr := os.Stat(filepath.Join(parent, "deployment", "docker-compose.yml")); statErr == nil {
		return parent, nil
	}
	return "", fmt.Errorf("could not locate repository deployment directory from %s", root)
}

func containsDependency(values []system.Dependency, name string) bool {
...
func stageError(stage string, err error) error {
	return fmt.Errorf("installation failed at %s: %w; logs: run risknexus logs", stage, err)
}
...
func askYesNo(prompt string, fallback bool) bool {
	value := strings.ToLower(readLine(prompt))
	if value == "" {
		return fallback
	}
