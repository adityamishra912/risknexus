...
func ValidateRiskNexusHost(host string) error {
	if host == "" {
		return errors.New("RiskNexus host/IP address is required")
	}
	...
	for _, label := range strings.Split(host, ".") {
		if label == "" || len(label) > 63 || label[0] == '-' || label[len(label)-1] == '-' {
			return fmt.Errorf("invalid RiskNexus hostname %q", host)
		}
...
func PromptRiskNexusHost(in io.Reader, out io.Writer, existing string) (string, error) {
	...
	for {
		if existing == "" {
			fmt.Fprint(out, "Enter RiskNexus host/IP address: ")
		} else {
			fmt.Fprintf(out, "Enter RiskNexus host/IP address [%s]: ", existing)
		}
...
func Prompt(in io.Reader, out io.Writer, existing *MySQLConfig, passwordReader func() ([]byte, error)) (MySQLConfig, error) {
	...
	if username == "" {
		return MySQLConfig{}, errors.New("MySQL username is required")
	}
	...
	fmt.Fprintln(out)
	if err != nil {
		return MySQLConfig{}, fmt.Errorf("read MySQL password: %w", err)
	}
	if len(password) == 0 && existing != nil {
		password = []byte(existing.Password)
	}
	if len(password) == 0 {
		return MySQLConfig{}, errors.New("MySQL password is required")
	}
...
func Save(path string, value MySQLConfig) error {
	...
	if value.MySQLHostContainer != "" {
		containerHost = value.MySQLHostContainer
	}
	hostName := value.RiskNexusHost
	if hostName == "" && value.CyberNexusHost != "" {
		hostName = value.CyberNexusHost
	}
	if hostName == "" {
		hostName = value.Host
	}
	if err := ValidateRiskNexusHost(hostName); err != nil {
		return err
	}
