...
func VerifyHTTP(ctx context.Context, settings Settings) error {
	...
	if err != nil { return fmt.Errorf("build GLPI health request: %w", err) }
	response, err := http.DefaultClient.Do(request)
	if err != nil { return fmt.Errorf("GLPI is not reachable at %s: %w", settings.URL, err) }
	defer response.Body.Close()
	if response.StatusCode >= 400 { return fmt.Errorf("GLPI returned HTTP %d at %s", response.StatusCode, settings.URL) }
	return nil
}
...
func WriteAgentConfig(settings Settings) error {
	...
	if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil { return fmt.Errorf("create GLPI Agent config directory: %w", err) }
	if err := os.WriteFile(path, []byte(AgentConfig(settings)), 0644); err != nil { return fmt.Errorf("write GLPI Agent config: %w", err) }
	return nil
}
